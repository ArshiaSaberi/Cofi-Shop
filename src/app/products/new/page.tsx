...
export default function NewProducts() {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(12); // باید state باشه
  const [currentPage, setCurrentPage] = useState(1);
  const searchParams = useSearchParams();
  const [isres, setisres] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [columns, setColumns] = useState(3); // تعداد ستون‌ها برای گرید

  // ---------- useEffect ریسایز مثل کامپوننت اول ----------
  useEffect(() => {
    function handleResize() {
      const width = window.innerWidth;
      let cols = 3; // حالت پیش‌فرض
      if (width < 400) cols = 1;
      else if (width < 650) cols = 2;
      else if (width < 900) cols = 3; // تغییر برای سایز ۹۰۰
      else if (width < 1024) cols = 4; // از ۹۰۰ تا ۱۰۲۴، ۴ ستون
      else cols = 4; // برای سایزهای بزرگتر هم ۴ ستون
    ...
    async function getProducts() {
      try {
        ...
        if (min !== undefined && max !== undefined) {
          ...
          filteredProducts = filteredProducts.filter(
            (item) =>
              typeof item.finalPrice === "number" && item.finalPrice >= min
          );
        } else if (max !== undefined) {
          filteredProducts = filteredProducts.filter(
            (item) =>
              typeof item.finalPrice === "number" && item.finalPrice <= max
          );
        }
...
const gridClass = `grid grid-cols-4 gap-5 mt-10 grid-cols-${columns} 
${columns === 1 ? "max-[400px]:grid-cols-1" : ""} 
${columns === 2 ? "max-[650px]:grid-cols-2" : ""} 
${columns === 3 ? "max-[900px]:grid-cols-3" : ""} 
${columns === 4 ? "min-[1024px]:grid-cols-4" : ""}`; // تغییر max به min برای سایز 1024px
