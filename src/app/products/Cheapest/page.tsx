...
export default function Expensiveroducts(): JSX.Element {
  const [products, setProducts] = useState<ProductType[]>([]);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [itemsPerPage, setItemsPerPage] = useState(12); // تعداد آیتم در هر صفحه
  const [columns, setColumns] = useState(3); // تعداد ستون‌ها برای گرید
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [isRes, setIsRes] = useState(false); // برای چک کردن اینکه محصولی یافت شده است یا خیر
  ...
  // ---------- کلاس گرید داینامیک ----------
  const gridClass = `grid gap-5 mt-10 grid-cols-4 grid-cols-${columns} ${
